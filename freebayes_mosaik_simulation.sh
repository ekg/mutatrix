...
echo
mutatrix -r 0.0001 -a 1.5 -S sample -p 2 -n 10 $reference | tee $answers
...
for sample in $samples; do wgsim -1 70 -2 70 -R 0 -r 0 -N 30000 $sample.fa  $sample.read1.fa $sample.read2.fa; done
...
bamtools filter $(for file in *sorted.bam; do echo $file; done | sed -e "s/^/ -in /g") \
    ...
    | freebayes -C 2 --no-filters --mnps --indels -f $reference --stdin \
...
do
    ...
    compare-vcf $answers.gz $resultsfilt.gz
    echo
done
